...
- (void)sendRequest:(NBMRequest *)requestToSend completion:(void (^)(NBMResponse *))responseBlock {
    //Request
    printf("SendRequest.265\n");
    if (responseBlock) {
        printf("SendRequest.265: Have ResponseBlock\n");
        NSUInteger reqId = _requestId++;
        printf("SendRequest.265[%s]: Have ResponseBlock\n", reqId);
...
- (void)sendRequestPack:(NBMRequestPack *)requestPack retried:(BOOL)retried
{
    ...
    printf("sendRequestPack: Start\n");
    if (retried) {
        printf("sendRequestPack: Retryed\n");
    ...
    printf("sendRequestPack: ::request\n");
